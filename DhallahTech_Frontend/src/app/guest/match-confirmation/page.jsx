        ...
        {!isVerifiable ? (
          <div className="space-y-4">
            ...
            <div className="hintText">
              {lang === "ar" ? "يرجى الإطلاع على بيانات مكتب المفقودات لإستلام العنصر ." : "Please contact the office to confirm pickup."}
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
            <button
                className="smallBtn"
                onClick={() =>
                router.push(`/guest/pickup-info?matchId=${matchId}&foundItemId=${foundItemId || ""}`)
              }
              >
              {lang === "ar" ? "بيانات المكتب" : "Contact Office"}
               </button>
              <button className="ghostBtn" onClick={() => router.push("/guest/matches")}>
                {lang === "ar" ? "رجوع" : "Back"}
              </button>
            </div>
          </div>
  ...
  <div className="animate-bounce" style={{ marginTop: 20 }}>
    ...
    <button
      ...
      onClick={() => {
        ...
        router.push(`/guest/pickup-info?matchId=${matchId}`);
      }}
    >
      {lang === "ar" ? "الانتقال لمعلومات الاستلام" : "Go to Pickup Info"}
    </button>
  </div>

            ) : (
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
                <button className="smallBtn" onClick={handleVerify} disabled={isLocked}>
